...
| -------- | --------------------- | ----------- | ------------------------------------------------------------ |
|          |                       |             |                                                              |
| 01.10.02 | 10/2/2025 -           | Yes         | Second attempt using Blazor Server and WebAssembly (Auto interactive mode, per page). The goal is to leverage C# as much as possible over JavaScript (nothing wrong with the latter, but sticking to a single language makes it easier). Created with Visual Studio 2026. |
| 01.10.01 | 8/22/2025 - 10/2/2025 | Yes         | Initial attempt using Blazor Server (Server interactive render mode) with JavaScript for user interactive. Once attempting the roadmap, decided to change directions. Had used Visual Studio 2022 Preview with .NET 10 SDK Preview to start (before Visual Studio 2026 public preview was available - then switched to this IDE) |
